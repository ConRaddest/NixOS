...
FloatingWindow {
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                fetchProcess.command = ["bash", "-c", "find " + shell.shellQuote(shell.themeDir) + " -mindepth 1 -maxdepth 1 -type d" + " | sed 's|.*/||' | sort"];
    ...
    function refresh() {
        ...
        currentProcess.command = ["bash", "-c", "readlink " + shell.shellQuote(shell.themeDir + "/current.nix") + " | sed 's|.*/||; s|/theme\\.nix$||'"];
    ...
    MenuPreview {
        ...
        highlightedText: function (text) {
            return themePicker.shell.highlightedText(text);
        }
