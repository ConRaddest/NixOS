#!/usr/bin/env bash
# Emits a single pipe-delimited line: CPU|RAM|WIFI|BLUETOOTH|BATTERY
# Consumed by shell.qml's statusProcess on a 1s timer.
...
WIFI_UP="false"
if [ -f "/sys/class/net/$WIFI_INTF/operstate" ] && [ "$(cat /sys/class/net/$WIFI_INTF/operstate)" = "up" ]; then
    WIFI_UP="true"
fi
...
# Battery (10% buckets, separate glyph set when charging)
