...
reload_cmd='bash -lc '\''source "$NOS_DIR/scripts/nos-apps.sh"; search_apps_with_selected_file "$1" "$2"'\'' _ {q} '"$selected_file_q"
toggle_cmd='bash -lc '\''source "$NOS_DIR/scripts/nos-apps.sh"; toggle_selected_app "$1" "$2"'\'' _ '"$selected_file_q"' {1}'
...
fzf_args=(
  ...
  --track
  --id-nth 1
  --preview 'bash -lc '\''printf "%s\n\n" "$1"; source "$NOS_DIR/scripts/nos-apps.sh"; package_preview "$2"'\'' _ {2} {1}'
